...
function Product() {
 ...
 const[description,setDescription]=useState("")
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (sizes) {
        parseSize = JSON.parse(sizes); // Only parse if sizes is not empty
      }
      if (colors) {
        parseColors = JSON.parse(colors); // Only parse if colors is not empty
      }
    } catch (error) {
      alert("Invalid JSON input for sizes or colors. Please check your input.");
      return; // Stop the submission if JSON parsing fails
    }
...
/>
  ...
  <div className="product-form w-1/3 bg-white p-6 rounded-lg shadow-md">
    ...
    <div className="flex flex-col gap-4">
      ...
      {/* Image Upload */}
      <CldUploadWidget uploadPreset="fgl3bmtq" onSuccess={handleUpload("thumbnail")}>
        {({ open }) => (
          <button
            type="button"
            onClick={open}
      ...
      <CldUploadWidget uploadPreset="fgl3bmtq" onSuccess={handleUpload("sideview")}>
        {({ open }) => (
          <button
            type="button"
            onClick={open}
