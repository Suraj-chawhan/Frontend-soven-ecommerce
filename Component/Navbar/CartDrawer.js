...
function CartDrawer({product,totalPrice,updateQuantity,Remove}) {
  ...
  const cartRef=useRef()
    useEffect(()=>{
        if(isToggled){
         gsap.fromTo(cartRef.current.children,{y:0,opacity:0},{y:10,opacity:1,delay:0.2})
     }
    },[isToggled])
...
function call(){
  ...
  return (
    <div className={`fixed top-0 right-0 md:w-[800px] w-[100%] overflow-scroll h-full bg-white shadow-lg transition-transform transform ${isToggled ? 'translate-x-0' : 'translate-x-full'}`} ref={cartRef} >
    <div className="p-4">
      <h2 className="text-xl font-bold">Bag</h2>
      <button className="absolute top-4 right-4 text-xl" onClick={() => dispatch(setFalse())}>
        Close Cart
      </button>
      ...
      <button className="mt-4 w-full bg-blue-500 text-white py-2 rounded" onClick={call}>Proceed to Checkout</button>
    </div>
  </div>
  )
}
