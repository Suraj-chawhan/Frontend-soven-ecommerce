...
export async function DELETE(req, { params }) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: error.message || "Failed to delete Wishlist" }),
      { status: 500 }
    );
  }
}
...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    const updatedWishlist = await Wishlist.findByIdAndUpdate(id, body, {
      new: true,
    });
    if (!updatedWishlist) {
      return new Response(JSON.stringify({ error: "Wishlist not found" }), {
    ...
    return new Response(
      JSON.stringify({ error: error.message || "Failed to update Wishlist" }),
      { status: 500 }
    );
  }
}
...
export async function GET(req, { params }) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: error.message || "Failed to fetch Wishlist" }),
      { status: 500 }
    );
  }
}
